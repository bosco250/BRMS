...
export default function Reservations() {
  const { reservations, tables, updateReservationStatus } =
    useWaiterDashboard();
  ...
  const totalCustomers = todayReservations.reduce(
      ...
      <div className="space-y-4">
        {filteredReservations.map((reservation) => {
          ...
          return (
            <div
              key={reservation.id}
              className="bg-white p-6 rounded-xl border border-border-primary hover:shadow-lg transition-all duration-200 hover:border-app-brand/30"
            >
              {/* Reservation Header */}
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-app-brand/10 rounded-xl flex items-center justify-center border border-app-brand/20 shadow-sm">
                    <Calendar className="w-7 h-7 text-app-brand" />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-text-muted bg-surface-secondary px-3 py-2 rounded-lg border border-border-primary">
                    {reservation.date}
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-3 bg-app-brand/5 rounded-lg border border-app-brand/20">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-xs text-app-brand">Time</span>
                    </div>
                    <span className="text-sm font-bold text-app-brand">
                      {reservation.time}
                    </span>
                  </div>
                </div>
              ...
              {tableInfo && (
                <div className="mb-6 p-4 bg-info/10 border border-info/20 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="p-2 bg-white/50 rounded border border-info/10">
                      <span className="font-medium">Capacity:</span>{" "}
                      {tableInfo.capacity} people
                    </div>
                    <div>
              ...
              <div className="flex gap-3">
                {reservation.status === "pending" && (
                  <>
                    <button
                      ...
                      className="px-6 py-3 bg-success text-white rounded-lg text-sm font-semibold hover:bg-success/90 transition-all duration-200 border border-success shadow-sm hover:shadow"
                    >
                    ...
                    <button
                      ...
                      className="px-6 py-3 bg-error text-white rounded-lg text-sm font-semibold hover:bg-error/90 transition-all duration-200 border border-error shadow-sm hover:shadow"
                    >
                ...
                {reservation.status === "confirmed" && (
                  <button
                    ...
                    className="px-6 py-3 bg-info text-white rounded-lg text-sm font-semibold hover:bg-info/90 transition-all duration-200 border border-info shadow-sm hover:shadow"
                  >
                    <CheckCircle className="w-4 h-4 mr-2 inline" />
                    Mark as Completed
                  </button>
                ...
                <button className="px-6 py-3 bg-surface-secondary text-text-primary rounded-lg text-sm font-semibold hover:bg-surface-primary transition-all duration-200 border border-border-secondary shadow-sm hover:shadow">
                  Edit Details
                </button>
              </div>
            </div>
