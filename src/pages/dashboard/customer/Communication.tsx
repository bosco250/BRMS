...
import { Mail, MessageSquare, Bell, Smartphone, Save, Check } from "lucide-react";
            ...
            <div key={key} className="flex items-center justify-between p-4 bg-surface-secondary rounded-lg">
              <div>
                ...
                <p className="text-xs text-text-secondary">
                  ...
                  {key === 'reservationReminders' && 'Receive reminders for upcoming reservations'}
                  {key === 'promotionalOffers' && 'Receive special deals and discounts'}
                  {key === 'loyaltyUpdates' && 'Get updates about your loyalty points and rewards'}
                  {key === 'newMenuItems' && 'Be notified when new menu items are available'}
                  {key === 'specialEvents' && 'Receive information about special events and promotions'}
                </p>
              </div>
              <button
                onClick={() => handleNotificationToggle(key as keyof typeof notificationSettings)}
                className={`w-12 h-6 rounded-full relative transition-colors ${
                  value ? 'bg-brand' : 'bg-surface-card'
                }`}
              >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-surface-secondary rounded-lg">
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full relative transition-colors ${
                communicationChannels.email ? 'bg-brand' : 'bg-surface-card'
              }`}
            >
              <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${
          ...
          <div className="flex items-center justify-between p-4 bg-surface-secondary rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-sm font-medium text-text-primary">SMS</span>
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full relative transition-colors ${
                communicationChannels.sms ? 'bg-brand' : 'bg-surface-card'
              }`}
            >
              <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${
          ...
          <div className="flex items-center justify-between p-4 bg-surface-secondary rounded-lg">
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full relative transition-colors ${
                communicationChannels.push ? 'bg-brand' : 'bg-surface-card'
              }`}
            >
              <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${
      ...
      <div className="p-6 rounded-lg border border-border-primary bg-dashboard">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="bg-surface-secondary text-text-primary px-4 py-2 rounded-lg hover:bg-surface-card transition-colors flex items-center gap-2"
          >
